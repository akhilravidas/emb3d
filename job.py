...
def gen_batch(job: EmbedJob, batch_size: int, max_tokens: int) -> Iterator[Batch]:
    ...
    for line_num, record in enumerate(reader.jsonl(job.in_file)):
        ...
        can_merge = len(batch_ids) == 0 or batch_token_count + new_tokens < max_tokens or len(batch_ids) < batch_size
        if can_merge:
    ...
    while num_retries > 0:
        ...
        match resp:
            ...
            case WaitFor(seconds, error):
                batch.error = error
                if num_retries != 0:
                    print(f"Sleep for {seconds} seconds before retrying...")
...
async def flush(run: EmbedJob, queue: asyncio.Queue):
    ...
    with open(run.out_file, "a") as fp:
        while True:
            ...
            for idx in range(len(result.row_ids)):
                fp.write(json.dumps({
...
# TODO: Return job stats (success, failed, skipped)
